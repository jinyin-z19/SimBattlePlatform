...
r and any other key: stop robot
...
if __name__ == "__main__":
    ...
    rospy.init_node(args.id + "teleop_twist_keyboard")

    # Walking Part
    pub = rospy.Publisher(args.id + "/cmd_vel", Twist, queue_size=1, latch=True)
    ...
    # Head Part
    rospy.Subscriber(args.id + "/joint_states", JointState, joint_state_cb, queue_size=1)
    head_pub = rospy.Publisher(args.id + "/walking_motor_goals", JointCommand, queue_size=1)
    ...
    walk_kick_pub = rospy.Publisher(args.id + "/kick", Bool, queue_size=1)
    joint_pub = rospy.Publisher(args.id + "/walking_motor_goals", JointCommand, queue_size=1)
    ...
    client = actionlib.SimpleActionClient(args.id + "/dynamic_kick", KickAction)
